...
pub use current::{
    current_process, current_task, current_task_id, current_task_id_may_uninit,
    current_task_may_uninit, current_token, CurrentTask,
};
pub use flags::{CloneFlags, TaskStatus};

pub use future::yield_now;
pub use id::{pid_alloc, PidHandle, RecycleAllocator};
pub use kstack::{current_stack_top, TaskStack};
pub use processor::{init, run_task2};
pub use schedule::{
    add_task, pick_next_task, put_prev_task, set_priority, task_tick, Task, TaskRef,
};
...
pub async fn exit_proc(exit_code: i32) {
    ...
    info!("[kernel] exit_group pid {}, exit_code: {}", pid, exit_code);
    ...
    *process.state.lock().await = TaskStatus::Zombie;
    ...
    for thread in tasks_to_terminate.iter() {
        ...
        if let Some(ptr) = thread.child_tid_ptr() {
            if process
                .manual_alloc_type_for_lazy(ptr as *const u32)
                .await
                .is_ok()
            {
...
pub async fn exit_current(exit_code: i32) {
    ...
    let process = current_process();
    debug!(
        "[kernel] exit_current tid {}, exit_code: {}",
        task.id(),
        exit_code
    );
    // 检查是否是进程中的最后一个线程
    ...
    if is_lead {
        ...
        exit_robust_list_cleanup(task.robust_list.lock().await.head)
            .await
            .unwrap();
...
async fn exit_robust_list_cleanup(head_ptr: usize) -> GeneralRet {
    ...
    for i in 0..MAX_ROBUST_LIST_NODES {
        ...
        let head_data: UserRobustListHead = match get_target_ref(token, current_head_ptr) {
            // 使用 get_target_data 获取拷贝
            Ok(data) => *data,
            Err(_) => {
                ...
                if (futex_val & FUTEX_TID_MASK) == current_task_tid as u32 {
                    ...
                    if put_data(token, pending_lock_addr as *mut u32, new_futex_val).is_ok() {
                        if (futex_val & FUTEX_WAITERS) != 0 {
                            ...
                            if let Some(wait_queue) =
                                futex_guard.get_mut(&(token, pending_lock_addr))
                            {
        ...
        while current_node_ptr != head_ptr {
            ...
            let lock_addr =
                (current_node_ptr as isize).wrapping_add(head_data.futex_offset) as usize;
...
pub fn add_initproc(cwd: &str, exe: &str, argv: &str) {
    ...
    let binding = get_args(format!("{} {} ", exe, argv).as_bytes());
